...
def WaitForReply(rocket, waittime, totaltime, chatid):
    ...
    while time.time() < mustend:
        ...
        try:
            ...
            if Extra_Commands_Module.checklastmessage(messages, waittime) == False:
                time.sleep(waittime)
            else:
                # As a heuristic, we're going to look at the first message that was posted after the chatbot's most recent post
                recent_message = Extra_Commands_Module.firstresponse(messages)
                break
        except KeyError:
            time.sleep(waittime)
